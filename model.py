...
class PiCO(nn.Module):
    ...
    def _dequeue_and_enqueue(self, keys, labels, partial_Y, args):
        ...
        end = ptr + batch_size if ptr + batch_size <= args.moco_queue else args.moco_queue
        # replace the keys at ptr (dequeue and enqueue)
        self.queue[ptr:end, :] = keys[0:end - ptr, :]
        self.queue_pseudo[ptr:end, :] = labels[0:end - ptr, :]
        self.queue_partial[ptr:end, :] = partial_Y[0:end - ptr, :]
    ...
    def _batch_shuffle_ddp(self, x, y, p_y):
        ...
        *** Only support DistributedDataParallel (DDP) model. ***
        ...
        return x[idx_this], y[idx_this], p_y[idx_this], idx_unshuffle
    ...
    def _batch_unshuffle_ddp(self, x, y, p_y, idx_unshuffle):
        ...
        *** Only support DistributedDataParallel (DDP) model. ***
        ...
        return x[idx_this], y[idx_this], p_y[idx_this]

    def reset_prototypes(self, prototypes):
        self.prototypes = prototypes

    def forward(self, img_q, im_k=None, partial_Y=None, args=None, eval_only=False, update_queue=True):
