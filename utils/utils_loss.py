...
class partial_loss(nn.Module):
    ...
    def forward(self, outputs, index, task, memory=False):
        logsm_outputs = F.log_softmax(outputs, dim=1)
        final_outputs = logsm_outputs * self.confidence[task, index, :]
        if memory:
    ...
    def confidence_update(self, temp_un_conf, batch_index, batchY, task):
        with torch.no_grad():
            _, prot_pred = (temp_un_conf * batchY).max(dim=1)
            pseudo_label = F.one_hot(prot_pred, batchY.shape[1]).float().cuda().detach()
            self.confidence[task, batch_index, :] = self.conf_ema_m * self.confidence[task, batch_index, :]\
...
class SupConLoss(nn.Module):
    ...
    def forward(self, features, mask=None, batch_size=-1, memory=False):
